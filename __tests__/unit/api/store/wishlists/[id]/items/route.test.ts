...
import {createMockRequest, createMockResponse, createMockWishlistService, createMockProductService} from '../../../../../helpers/mock-route'
...
describe('POST /store/wishlists/:id/items', () => {
	it('should add item and return 201', async () => {
		const item = {id: 'wi_new', product_variant_id: 'variant_1', wishlist_id: 'wl_1'}
		const {req, res, wishlistService, productService} = setup({body: {product_variant_id: 'variant_1'}})
		wishlistService.retrieveWishlist.mockResolvedValue(WISHLIST)
		productService.retrieveProductVariant.mockResolvedValue({id: 'prod_1'})
		wishlistService.listWishlistItems.mockResolvedValue([])
		wishlistService.createWishlistItems.mockResolvedValue(item)

		await POST(req, res)

		expect(wishlistService.createWishlistItems).toHaveBeenCalledWith({product_variant_id: 'variant_1', wishlist_id: 'wl_1'})
	...
	it('should return 404 when wishlist belongs to another customer', async () => {
		const {req, res, wishlistService} = setup({customerId: 'other_cust', body: {product_variant_id: 'variant_1'}})
		wishlistService.retrieveWishlist.mockResolvedValue(WISHLIST)
	...
	it('should return 404 when product variant does not exist', async () => {
		const {req, res, wishlistService, productService} = setup({body: {product_variant_id: 'variant_missing'}})
		wishlistService.retrieveWishlist.mockResolvedValue(WISHLIST)
		productService.retrieveProductVariant.mockRejectedValue(new Error('Not found'))

		await POST(req, res)

		expect(res.status).toHaveBeenCalledWith(404)
		expect(res.json).toHaveBeenCalledWith({message: 'Product variant "variant_missing" not found'})
	})
	...
	it('should throw when not authenticated', async () => {
		const {req, res} = setup({customerId: null, body: {product_variant_id: 'variant_1'}})

...
import {DELETE} from '../../../../../../../../src/api/store/wishlists/[id]/items/[product_variant_id]/route'
...
const setup = (
	...
	const req = createMockRequest({
		...
		params: opts.params ?? {id: 'wl_1', product_variant_id: 'variant_1'},
		services: {[WISHLIST_MODULE]: wishlistService}
	})
	return {req, res, wishlistService}
}

describe('DELETE /store/wishlists/:id/items/:product_variant_id', () => {
	it('should remove item and return id', async () => {
		...
		wishlistService.listWishlistItems.mockResolvedValue([{id: 'wi_1', product_variant_id: 'variant_1', wishlist_id: 'wl_1'}])
	...
	it('should return 404 when product variant not in wishlist', async () => {
		const {req, res, wishlistService} = setup()
		wishlistService.retrieveWishlist.mockResolvedValue(WISHLIST)
		...
		expect(res.status).toHaveBeenCalledWith(404)
		expect(res.json).toHaveBeenCalledWith({message: 'Product variant "variant_1" not found in wishlist'})
		expect(wishlistService.deleteWishlistItems).not.toHaveBeenCalled()
	})
